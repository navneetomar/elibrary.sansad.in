import { getValueNameCookie } from './../../core/config/constants';
...
import { map, switchMap, take,distinct } from 'rxjs/operators';
import {
  buildPaginatedList,
  PaginatedList,
...
export class EPeopleRegistryComponent implements OnInit, OnDestroy {
  ...
  isEPersonFormImportShown: boolean;

  isEPersonFormImportToDeleteShown: boolean;
  ...
  initialisePage() {
    ...
    // this.isEPersonFormImportShown = false;
    this.search({
    ...
    this.subs.push(
      this.epersonService.getActiveEPerson().subscribe((eperson: EPerson) => {
        if (eperson && eperson.id) {
          this.isEPersonFormShown = true;
          // this.isEPersonFormImportShown = true;
        }
      })
    );
    this.subs.push(
      this.ePeople$
        .pipe(
          ...
          switchMap((epeople: PaginatedList<EPerson>) => {
            if (epeople && epeople.pageInfo && epeople.pageInfo.totalElements > 0) {
  ...
  /**
   * reset password EPerson, show notification on success/failure & updates EPeople list
   */
  resetPasswordEPerson(ePerson: EPerson) {
    if (hasValue(ePerson.id)) {
      const modalRef = this.modalService.open(ConfirmationModalComponent);
      modalRef.componentInstance.dso = ePerson;
      modalRef.componentInstance.headerLabel =
        'confirmation-modal.reset-password-eperson.header';
      modalRef.componentInstance.infoLabel =
        'confirmation-modal.reset-password-eperson.info';
      modalRef.componentInstance.cancelLabel =
        'confirmation-modal.reset-password-eperson.cancel';
      modalRef.componentInstance.confirmLabel =
        'confirmation-modal.reset-password-eperson.confirm';
        ...
        .subscribe((confirm: boolean) => {
          if (confirm) {
            if (hasValue(ePerson.id)) {
              ...
              this.http
                .post<any>(
                  `${this.appConfig.rest.baseUrl}/api/dladmin/accesscontrol/ep_resetpwdef?id=${ePerson.id}`,
                  {},
                  options
                )
                .subscribe({
                  next: (data) => {
                    this.notificationsService.success(
                      this.translateService.get(
                        this.labelPrefix + 'notification.resetpassword.success',
                        { name: ePerson.name }
                      )
                    );
                  },
                  error: (error) => {
                    ...
                    if (error && error.status === 200) {
                      ...
                      this.notificationsService.error(
                        'Error occured when trying to reset password EPerson with id: ' +
                          ePerson.id +
                          ' with code: ' +
                          error.status +
                          ' and message: ' +
  ...
  handleClickImport() {
    this.isEPersonFormShown = false;
    this.isEPersonFormImportShown = true;
    this.isEPersonFormImportToDeleteShown = false;
  }
  handleClickImportToDelete() {
    this.isEPersonFormShown = false;
    this.isEPersonFormImportShown = false;
    this.isEPersonFormImportToDeleteShown = true;
  }
  onReloadPage() {
    this.isEPersonFormShown = false;
    this.isEPersonFormImportShown = false;
