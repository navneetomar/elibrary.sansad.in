...
export class ObjectCollectionComponent implements OnInit {
  ...
  @Input() isHomePageTopLevelCommun = false;
  ...
  @Input() isBrowserBy: boolean;
  // @Input() isCollectionPage: boolean;
  /**
  ...
  @Output() isSearchPageOut;
  @Output() isBrowserByPageOut;
  @Output() isHomePageTopLevelCommunOut;
  ...
  ngOnInit(): void {
    this.currentMode$ = this.route.queryParams.pipe(
      map((params) =>
         isEmpty(params?.view) ? ((params?.configuration && (params?.configuration === 'workspace' || params?.configuration === 'workflow')) ?  ViewMode.TableElement : ViewMode.GridElement) : params.view
      ),
    ...
    this.isSearchPageOut = this.isSearchPage;
    this.isBrowserByPageOut = this.isBrowserBy;
    this.isHomePageTopLevelCommunOut = this.isHomePageTopLevelCommun;
  }
