 ...
 */
export class ResourcePoliciesComponent implements OnInit, OnDestroy, OnChanges {
  /**
   * The resource UUID
   * @type {string}
   */
  ...
  ngOnInit(): void {
    this.isActive = true;
    this.idPrev = this.resourceUUID;

    this.initResourcePolicyList();
  }
  ngOnChanges(): void {
    if (
      this.resourceUUID !== this.idPrev &&
      this.idPrev !== null &&
      this.idPrev !== undefined
    ) {
      this.initResourcePolicyList();
    }
  }
  ...
  redirectToResourcePolicyCreatePage(): void {
    if (this.isTablePolicy === false) {
      ...
      if (this.resourceType === COMMUNITY_TYPE) {
        this.router.navigate(
          [`/communities/${this.resourceUUID}/edit/authorizations/create`],
          {
            queryParams: {
              policyTargetId: this.resourceUUID,
              targetType: this.resourceType,
      ...
      } else if (this.resourceType === COLLECTION_TYPE) {
        this.router.navigate(
          [`/collections/${this.resourceUUID}/edit/authorizations/create`],
          {
            queryParams: {
              policyTargetId: this.resourceUUID,
              targetType: this.resourceType,
      ...
      } else if (this.resourceType === ITEM_TYPE) {
        this.router.navigate(
          [`/items/${this.resourceUUID}/edit/authorizations/create`],
          {
            queryParams: {
              policyTargetId: this.resourceUUID,
              targetType: this.resourceType,
      ...
      } else if (this.resourceType === BITSTREAM_TYPE) {
        this.router.navigate(
          [`/bitstreams/${this.resourceUUID}/authorizations/create`],
          {
            queryParams: {
              policyTargetId: this.resourceUUID,
              targetType: this.resourceType,
