...
import { Topic } from 'src/app/core/shared/topic.model';
...
export class CreateComColPageComponent<
  TDomain extends Collection | Community | Topic
> implements OnInit
{
  /**
  ...
  public constructor(
    ...
    protected parentDataTopicService: TopicDataService,
  ...
  ngOnInit(): void {
    ...
    this.parentUUIDTOPIC$ = this.routeService.getQueryParameterValue('parent');
    ...
    this.parentUUIDTOPIC$.pipe(take(1)).subscribe((parentID: string) => {
      if (isNotEmpty(parentID)) {
        this.parentRDTOPIC$ = this.parentDataTopicService.findById(parentID);
      }
    });
  }
      ...
      this.parentUUIDTOPIC$
        ...
        .subscribe((dsoRD: TDomain) => {
          if (isNotUndefined(dsoRD)) {
            ...
            this.dsoDataService.refreshCacheTopic(dsoRD);
          }
          this.notificationsService.success(
            null,
            this.translate.get(
              this.type.value + '.create.notifications.success'
            )
          );
        });
    } else {
