...
export class PaginationComponent implements OnDestroy, OnInit, AfterViewInit {
  ...
  viewMode: ViewMode = ViewMode.GridElement;
  ...
  @Input() isSearchPageOut = false;
  @Input() isBrowserByPageOut = false;
  @Input() isHomePageOut = false;
  @Input() isShowViewMode = false;
  ...
  @Input() public isEpeopleRegistry = false;
  @Input() public isGroupsRegistry = false;
  @Input() public isMetadataRegistry = false;
  @Input() public isBitstreamFormat = false;
  @Input() public isProcessOverview = false;
  ...
  ngOnInit() {
    ...
    this.checkHasSortField();
    if (this.location && (this.location.path().includes('/mydspace') || this.location.path().includes('/access-control/groups/'))){
  ...
  public doPageChange(page: number) {
    ...
    if (this.isHomePageOut) {
      setTimeout(() => {
        this.setScrollPosition(1261);
      }, 500);
    }
  }
  ...
  ngAfterViewInit() {
    // console.log('ngAfterViewInit page');
  ...
  checkHasSortField(){
    if (typeof window !== 'undefined') {
      ...
      const listHas = ['search','communities','collections'];
      listHas.forEach(item=>{
        if(splitUrl[0].includes(item)){
          this.hasSortField = true;
        }
      })
    }
  }
  ...
  public doSortDirectionChange(sortDirection: SortDirection) {
    if(this.hasSortField){
      ...
      this.emitPaginationChange(sortDirection);
    }
    else{
      this.updateParams({
        pageId: this.id,
   ...
   */
  private emitPaginationChange(sortDirection?: any) {
    this.paginationChange.emit(sortDirection);
  }
  ...
  updatePagination(value: number) {
    this.paginationService
      ...
      .subscribe((currentPaginationOptions) => {
        this.updateParams({
          page: (currentPaginationOptions.currentPage + value).toString(),
        });
      });
  }
}
