...
export class DsDynamicFormControlContainerComponent extends DynamicFormControlContainerComponent implements OnInit, OnChanges, OnDestroy {
   ...
   */
  ngOnInit(): void {
    // console.log("formId",this.formId,this.item,this.location.path());
    ...
    /* set dc.collection.map default for item */
    // if (typeof window === 'object' && hasValue(window.localStorage)) {
    //   if(this.location.path() && this.location.path().includes('/workspaceitems') && this.location.path().includes('/edit')){
    //     let collection  = window.localStorage.getItem('itemCollection');
    //     const locationPath = this.location.path().split('/');
  
    //     if(this.model && this.model.id && this.model.id === 'dc_collection_mapping' && locationPath && locationPath[2]){
    //       this.http.patch(`${this.appConfig.rest.baseUrl}/api/submission/workspaceitems/${locationPath[2]}`,
    //       [{"op":"add","path":"/sections/traditionalpageone/dc.collection.mapping","value":[{"value":collection,"language":null,"authority":null,"display":collection,"confidence":-1,"place":0,"otherInformation":null}]}]
    //       ).subscribe(data=>{
  ...
  /* end set dc.collection default for item */  
  }
