...
export class DSOSelectorComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.typesString = this.types
      .map((type: string) =>
        this.translate.instant(type.toString().toLowerCase())
      )
      .join(', ');
    ...
    if (isNotEmpty(this.currentDSOId)) {
      ...
      currentDSOResult$.subscribe((data) => {
        
        if (data && data.page && data.page[0] && data.page[0]?.type.toString() === 'topic'){
          this.http
          .get(
            `${this.appConfig.rest.baseUrl}/api/core/topics/${data.page[0].indexableObject.id}/mainTopic`
          )
          .subscribe((data1) => {
            if (data1) {
              data.page[0].isMainTopic = false;
            } else {
              data.page[0].isMainTopic = true;
            }
          });
        }
      });
                  ...
                  if (rd.hasSucceeded) {
                    ...
                    rd.payload.page = [
                      ...(isEmpty(query) && page === 1
                        ...
                        : []),
                      ...rd.payload.page.filter((result) => {
                        if (this.isTopic) {
                          if (result.indexableObject.id !== this.currentDSOId) {
                            this.http
                              .get(
                                `${this.appConfig.rest.baseUrl}/api/core/topics/${result.indexableObject.id}/mainTopic`
                              )
                              .subscribe((data) => {
                                if (data) {
                                  result.isMainTopic = false;
                                } else {
                                  result.isMainTopic = true;
                                }
                              });
                          }
                        }
  ...
  handleClickItem(listEntry) {
    // if(listEntry && listEntry.indexableObject && listEntry.indexableObject.type === 'collection' && listEntry.indexableObject._name){
    //   if(typeof window === 'object' && hasValue(window.localStorage)){
    //     window.localStorage.setItem('itemCollection',listEntry.indexableObject._name);
    //   }
    // }
    this.onSelect.emit(listEntry.indexableObject);
    
  }
  checkMainTopic(id): Observable<boolean> {
    return this.http
      .get<boolean>(
        `${this.appConfig.rest.baseUrl}/api/core/topics/${id}/mainTopic`
      )
      .pipe(
        map((data) => {
          if (data) {
