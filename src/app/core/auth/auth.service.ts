...
import { Observable, fromEvent, of as observableOf, timer, merge, BehaviorSubject, Subject } from 'rxjs';
...
import { NONE_ENTITY_TYPE } from '../shared/item-relationships/item-type.resource-type';
import { IsUserActiveService } from '../isUserActive/isUserActive.service';
...
export class AuthService {
   ...
   * @param {any} passPhrase The user's password
    ...
    return this.authRequestService.postToEndpoint('login', body, options).pipe(
      map((rd: RemoteData<AuthStatus>) => {
        ...
        if (hasValue(rd.payload) && rd.payload.authenticated) {
          ...
          //  this.isUserActive.userInactive.subscribe(isIdle=>this.isUserActiveFlag = isIdle)
          return rd.payload;
        }
        else {
          if (rd.payload.error.message == "User already has an active session") {
            throw (new Error('Sorry, you already have an active session.'));

          }
          else if (rd.payload.error.message.includes("User is locked out. Remaining lockout time: ")) {
            throw (new Error('Attempts limit reached. Try after 20 minutes'));
          }
          else {
  ...
  //   this.isUserActiveFlag = false;
  //   this.isUserActive.reset();
  // }
  ...
  public navigateToRedirectUrl(redirectUrl: string) {
    ...
    if (!hasValue(redirectUrl) || !redirectUrl.includes('reload/')) {
      ...
      if (redirectUrl && redirectUrl?.toString().includes('/bitstreams/') && redirectUrl?.toString().includes('/view')) {
        if (typeof localStorage !== 'undefined' && localStorage?.getItem('pathItemHistory')) {
          this.hardRedirectService.redirect(localStorage.getItem('pathItemHistory'));
          localStorage.removeItem('pathItemHistory');
        } else {
          this.hardRedirectService.redirect('/home');
        }
      } else {
  ...
  impersonate(epersonId: string) {
    this.storage.set(IMPERSONATING_COOKIE, epersonId, { secure: true, httpOnly: true, sameSite: "none" });
