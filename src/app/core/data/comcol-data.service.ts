...
export abstract class ComColDataService<
    T extends Community | Collection | Topic
  >
    ...
    DeleteData<T>
{
  ...
  public getBrowseEndpointTopic(
    ...
    if (isEmpty(options.scopeID)) {
      ...
      return scopeCommunityHrefObs.pipe(
        switchMap((href: string) => this.rdbService.buildSingle<Topic>(href)),
        getFirstCompletedRemoteData(),
        map((response: RemoteData<Topic>) => {
          if (response.hasFailed) {
            throw new Error(
              `The Topic with scope ${options.scopeID} couldn't be retrieved`
            );
          } else {
            return response.payload._links[linkPath];
          }
  ...
  public refreshCacheTopic(dso: T) {
    const parentTopicUrl = this.parentTopicUrlLookup(dso as any);
    if (!hasValue(parentTopicUrl)) {
      return;
    }
    observableCombineLatest([
      this.findByHref(parentTopicUrl).pipe(getFirstCompletedRemoteData()),
      this.halService.getEndpoint('topics/search/top').pipe(take(1)),
    ]).subscribe(([rd, topHref]: [RemoteData<any>, string]) => {
      if (
        rd.hasSucceeded &&
        isNotEmpty(rd.payload) &&
  ...
  private parentTopicUrlLookup(dso: Topic) {
    const parentTopic = dso._links.mainTopic;
    return isNotEmpty(parentTopic) ? parentTopic.href : null;
  }
  /**
